...
function Home() {
  ...
  useEffect(() => {
    const sectionIds = ['testimonials', 'how-it-works-section', 'features', 'pricing', 'faq', 'cta'];
  ...
  return (
    <div className={`min-h-screen bg-gray-50 ${heebo.className}`}>
      {/* Deploy-check banner — remove after confirming Vercel picks up changes */}
      <div className="fixed top-0 left-0 right-0 z-[999] bg-amber-500 text-white text-center text-xs py-1 font-bold">
        ✅ גרסה מעודכנת — אשף המסחר v2
      </div>
      {/* Header */}
      ...
      <nav className="fixed left-4 top-1/2 -translate-y-1/2 z-40 hidden lg:flex flex-col gap-4" dir="ltr">
        {[
          ...
          <button
            ...
            aria-label={label}
          >
            <span className={`flex-shrink-0 w-2.5 h-2.5 rounded-full border-2 transition-all duration-300 ${
              activeSection === id
                ? 'bg-amber-500 border-amber-500 scale-125'
                : 'bg-transparent border-gray-400 group-hover:border-amber-400 group-hover:scale-110'
            }`} />
